...
# LEFT JOIN
 ...
 SELECT p.property_id, p.name AS property_name, r.review_id, r.rating, r.comment, u.first_name AS reviewer_first_name,u.last_name AS reviewer_last_name FROM Property AS p LEFT JOIN Review AS r ON p.property_id = r.property_id LEFT JOIN  User AS u ON r.user_id = u.user_id ORDER BY p.name;
# FULL OUTER JOIN
